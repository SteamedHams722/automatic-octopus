...
def responses_to_pg(sheet_name):
    ...
    responses_data = responses_json.replace(
        "'", "''"
        ...
        with conn.cursor() as cursor:
            ...
            # used to expand them out further.
                ...
                try:
                    ...
                    values ('{3}','{4}', '{5}');""".format(
                        db, schema, table, user_id, responses_data, timestamp
                    )
                    ...
                    # the previous data
                    delete_old = (
                        "delete from {0}.{1}.{2} where created_on_utc < '{3}'".format(
                            db, schema, table, timestamp
                        )
                    )
